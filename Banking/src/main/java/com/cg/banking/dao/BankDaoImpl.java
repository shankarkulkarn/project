...
public class BankDaoImpl implements BankDao{
	...
	public List<AccTransaction> getTransactions(String accId, int no) {
		String jpql ="from AccTransaction tx inner join fetch tx.account a inner join fetch a.customer c where a.accountId=:accid order by desc tx.transaccountId";
	...
	public List<AccTransaction> getTransactions(String accId, LocalDate fromdt, LocalDate todt) {
		String jpql ="from AccTransaction tx inner join fetch tx.account a inner join fetch a.customer c where a.accountId=:accid and tx.transDate between :from and :to order by desc tx.transaccountId";
		TypedQuery<AccTransaction> query = em.createQuery(jpql, AccTransaction.class);
		query.setParameter("accid", accId);
